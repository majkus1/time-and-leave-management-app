...
export default function CustomPackageModal({ isOpen, onClose, lang = 'pl' }: CustomPackageModalProps) {
	...
	const totalPrice = calculateCustomTotalPrice(usersCount, selectedFeatures, packageType, lang)
	...
	const handleSelectAll = () => {
		if (selectedFeatures.length === AVAILABLE_FEATURES.length) {
			// Deselect all
			setSelectedFeatures([])
		} else {
			// Select all
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const validation = validateCustomPackageRequest({ 
			email, 
			companyName, 
			usersCount, 
			packageType: t.packageType[packageType],
			selectedFeatures 
		})
		...
		try {
			const packageTypeText = t.packageType[packageType]
			const totalPriceText = formatCustomPrice(totalPrice, lang)

			await axios.post(`${API_URL}/api/public/request-custom-package`, {
				packageType: packageTypeText,
				usersCount,
				selectedFeatures,
		...
		>
			...
			>
				...
				<div className="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6 rounded-t-2xl">
					...
					<p className="mt-3 text-sm custom-package-description">{t.description}</p>
				</div>
				...
				<form onSubmit={handleSubmit} className="p-6 space-y-6">
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-3">
							{t.packageType.label} <span className="text-red-500">*</span>
						</label>
					...
					<div>
						<div className="flex items-center justify-between mb-3">
							<label className="block text-sm font-semibold text-gray-700">
								{t.features.label} <span className="text-red-500">*</span>
							</label>
							<button
								type="button"
							...
							>
								{selectedFeatures.length === AVAILABLE_FEATURES.length 
									? (lang === 'pl' ? 'Odznacz wszystkie' : 'Deselect all')
									: t.features.selectAll}
							</button>
						</div>
						<p className="text-xs text-gray-500 mb-3">{t.features.minimumRequired}</p>
							...
							{AVAILABLE_FEATURES.map((feature) => {
								...
								return (
									...
									>
										...
										/>
										<span className="ml-3 text-gray-700 font-medium custom-feature-name">
											{getFeatureName(feature.id, lang)}
										</span>
									</label>
								)
							})}
					...
					<div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-4 border border-green-200">
						<div className="flex items-center justify-between">
							...
							<span className="text-3xl font-bold text-green-600">
								{formatCustomPrice(totalPrice, lang)}
								{dedicatedEnvironment && (
						...
						<p className="text-xs text-gray-500 mt-1">
							{usersCount} {lang === 'pl' ? 'użytkowników' : 'users'} × {formatCustomPrice(pricePerUser, lang)} = {formatCustomPrice(totalPrice, lang)}
							{dedicatedEnvironment && (
					...
					{submitMessage && (
						<div className={`p-4 rounded-lg ${
							submitMessage.includes('Dziękujemy') || submitMessage.includes('Thank you') 
