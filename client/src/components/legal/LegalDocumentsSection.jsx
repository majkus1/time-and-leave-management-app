...
/**
 * Blok dokumentów prawnych (akceptacje + linki) — używany na stronie Pakiety oraz osobno z loaderem.
 */
export default function LegalDocumentsSection() {
	...
	const handleAcceptDocuments = async () => {
		setAccepting(true)
		try {
			const documentsToAccept = Object.keys(acceptanceStatus.requiredDocuments).filter(
				type => !acceptanceStatus.requiredDocuments[type].isAccepted
			)
	...
	return (
		<div
			className="packages-legal-inner"
			style={{
				maxWidth: '1200px',
				margin: '0 auto',
			}}
		>
			<div className="logs-header" style={{ marginBottom: '24px', textAlign: 'center' }}>
				...
				>
					<img src="/img/docs.png" alt="" style={{ marginRight: '10px', verticalAlign: 'middle' }} />{' '}
					{t('legal.title')}
				</h2>
				<hr />
			</div>
			...
			>
				...
				{!needsAcceptance && (
					...
					>
						<div style={{ display: 'flex', alignItems: 'center' }}>
							<svg
								style={{ width: '20px', height: '20px', color: '#10b981', marginRight: '8px' }}
								viewBox="0 0 20 20"
								fill="currentColor"
							>
								<path
							...
							<span style={{ fontSize: '14px', fontWeight: '500', color: '#065f46' }}>{t('legal.allAccepted')}</span>
						</div>
					</div>
				)}

				<div style={{ marginTop: '24px' }}>
					<h2 style={{ padding: '15px', fontSize: '20px', fontWeight: '600', color: '#111827', marginBottom: '16px' }}>
						{t('legal.availableDocuments')}
					</h2>
								...
								<div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
									<div style={{ flex: 1 }}>
										...
										<p style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>
											{t('legal.version')} {doc.currentVersion} • {t('legal.effectiveFrom')}{' '}
											{new Date(doc.effectiveAt).toLocaleDateString(i18n.language === 'en' ? 'en-US' : 'pl-PL')}
										</p>
				...
				<div style={{ padding: '15px', marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e5e7eb' }}>
					<p style={{ fontSize: '14px', color: '#6b7280' }}>{t('legal.publiclyAvailable')}</p>
				</div>
			</div>
		</div>
	)
