...
const { isBlockingLeaveRequest, isHourlyLeaveRequest } = require('../utils/leaveSettlement')
const {
	canSupervisorApproveLeaves,
	canSupervisorViewTimesheets,
	canManageScheduleEntries,
} = require('./roleService')
const { collectVisibleScheduleEntries } = require('../utils/scheduleVisibility')
const {
...
async function buildWorkSummary({ viewer, scope, todayStart, todayEnd, monthStart, monthEnd, todayContext }) {
	...
	// Zatwierdzony urlop nie zakłada wpisu w ewidencji, więc bez tego zapytania osoby
	// na urlopie wliczałyby się do „brak wpisu dzisiaj" i zawyżały liczbę do popędzenia.
	// Zapytanie jest osobne od tego w podsumowaniu urlopów — tamto obejmuje cały zespół,
	// a tutaj interesuje nas wyłącznie zakres ewidencji oglądającego.
	const leavesToday = timesheetUserIds.length
		? await LeaveRequest.find({
			...
			.select('userId daysRequested hoursRequested settlementUnit')
			.lean()
		: []

	// Tylko nieobecność całodniowa zwalnia z ewidencji — wniosek godzinowy nie.
	const excusedToday = leavesToday.filter(isBlockingLeaveRequest).map(request => String(request.userId))
	...
	return {
		...
		teamMonth: {
			...
			todayMissingCount: countMissingWorkdayEntries({
				scopeUserIds: timesheetUserIds,
				recordedUserIds: todayEntriesByUser,
				excusedUserIds: excusedToday,
				isWorkingDay: todayContext?.isWorkingDay !== false,
			}),
		},
	}
}
...
// Ile dni do przodu pokazujemy pracownikowi jego własny grafik.
const OWN_SCHEDULE_LOOKAHEAD_DAYS = 7

async function buildScheduleSummary({ viewer, scope, enabled, todayStart, todayEnd }) {
	if (!enabled) {
		return { enabled: false, todayEntriesCount: 0, draftEntriesCount: 0, todayEntries: [], myUpcoming: [] }
	}

	const roles = Array.isArray(viewer.roles) ? viewer.roles : []
	const broadScope = roles.includes('Admin') || roles.includes('HR') || scope.isSupervisor
	const visibleIds = new Set(
		broadScope ? scope.timesheetUsers.map(u => String(u._id)) : [String(viewer._id)]
	)
	// type, createdBy, departmentName i members są potrzebne do rozstrzygnięcia,
	// czy oglądający zarządza grafikiem — czyli czy wolno mu widzieć szkice.
	const schedules = await Schedule.find({ teamId: viewer.teamId, isActive: true })
		.select('name days teamId type createdBy departmentName members')
		.lean()

	// Szkice pokazujemy wyłącznie w grafikach, którymi oglądający zarządza — tą samą
	// regułą co moduł grafików, żeby dashboard nie omijał bramki publikacji.
	const manageableScheduleIds = new Set()
	for (const schedule of schedules) {
		if (await canManageScheduleEntries(viewer, schedule)) {
			manageableScheduleIds.add(String(schedule._id))
		}
	}

	const todayEntries = collectVisibleScheduleEntries(schedules, {
		fromDate: todayStart,
		toDate: todayEnd,
		employeeIds: visibleIds,
		manageableScheduleIds,
	})

	// Własny grafik na kolejne dni — żeby pracownik wiedział, na kiedy jest jutro,
	// bez wchodzenia w moduł grafików.
	const myUpcoming = collectVisibleScheduleEntries(schedules, {
		fromDate: startOfDay(addDays(todayStart, 1)),
		toDate: endOfDay(addDays(todayStart, OWN_SCHEDULE_LOOKAHEAD_DAYS)),
		employeeIds: new Set([String(viewer._id)]),
		manageableScheduleIds,
	})
	...
	return {
		...
		draftEntriesCount: todayEntries.filter(entry => entry.isPublished === false).length,
		todayEntries: todayEntries.slice(0, 6),
		myUpcoming: myUpcoming.slice(0, OWN_SCHEDULE_LOOKAHEAD_DAYS),
	}
}
...
async function buildTeamInsights(userId, options = {}) {
	...
	// Każda sekcja zawęża się do SWOJEGO uprawnienia — zakres oglądającego jest sumą
	// dwóch różnych zbiorów i nie wolno użyć go wprost do żadnej z nich.
	const workTargetUsers = filterUsersInScope(targetUsers, scope.timesheetUsers)
	const workTargetIds = workTargetUsers.map(user => user._id)
	const leaveTargetUsers = filterUsersInScope(targetUsers, scope.leaveApprovalUsers)
	const leaveTargetIds = leaveTargetUsers.map(user => user._id)
	...
	if (modules.leaves && leaveTargetIds.length > 0) {
		// Oczekujące wnioski widać wyłącznie tam, gdzie oglądający je zatwierdza.
		// Wybranie kogoś filtrem nie może tego obejść — stąd leaveTargetIds, a nie targetIds.
		const pendingCount = await LeaveRequest.countDocuments({
			userId: { $in: leaveTargetIds },
			status: 'status.pending',
		})

		const approvedInPeriod = await LeaveRequest.find({
			userId: { $in: leaveTargetIds },
			status: { $in: ['status.accepted', 'status.sent'] },
		...
		const upcomingCount = await LeaveRequest.countDocuments({
			userId: { $in: leaveTargetIds },
			status: { $in: ['status.accepted', 'status.sent'] },
...
async function buildPersonalInsights({ viewer, teamSettings, settingsDoc, leavesEnabled }) {
	...
	const fullSettings = {
		...
		// Bez tego pola saldo zostałoby pomniejszone drugi raz: automat już je odjął,
		// a widok odjąłby jeszcze wykorzystane dni.
		autoDeductLeaveLimits: settingsDoc?.autoDeductLeaveLimits === true,
	}
...
async function buildDashboardSummary(userId) {
	...
	const settingsDoc = await Settings.findOne({ teamId: viewer.teamId })
		.select('timerEnabled workOnWeekends includePolishHolidays includeCustomHolidays customHolidays leaveRequestTypes leaveCalculationMode leaveHoursPerDay autoDeductLeaveLimits')
		.lean()
	const teamSettings = {
